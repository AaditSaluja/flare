...
PairFLAREB2sep::PairFLAREB2sep(LAMMPS *lmp) : Pair(lmp) {
  restartinfo = 0;
  manybody_flag = 1;
...
void PairFLAREB2sep::compute(int eflag, int vflag) {
...
void PairFLAREB2sep::settings(int narg, char ** /*arg*/) {
...
void PairFLAREB2sep::coeff(int narg, char **arg) {
  if (!allocated)
    allocate();
...
void PairFLAREB2sep::init_style() {
  // Require newton on.
...
double PairFLAREB2sep::init_one(int i, int j) {
...
void PairFLAREB2sep::read_file(char *filename) {
  int me = comm->me;
...
void PairFLAREB2sep::grab(FILE *fptr, int n, double *list) {
  char *ptr;
  char line[MAXLINE];
